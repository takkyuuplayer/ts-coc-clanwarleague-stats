...
const SUMMARY_SHEET_HEADER = [
    "Clan Name",
    "Clan Tag",
    "Members",
]

const CLAN_SHEET_HEADER = [
    "Player Name",
    "Player Tag",
    "TH",
    "Stars",
...
export class Coc {
    ...
    public static createSpreadsheetRequestBody(myClanTag: string, leaguegroup: any): sheets_v4.Schema$Spreadsheet {
        const myClan = leaguegroup.clans.find((clan: any) => clan.tag === myClanTag)
        return {
            properties: {
    ...
    public static initializeSummarySheetRequestBody(leaguegroup: any): sheets_v4.Schema$BatchUpdateValuesRequest {
        const column = String.fromCharCode('A'.charCodeAt(0) + MAX_TH_LEVEL + SUMMARY_SHEET_HEADER.length - 1)
        const townhallLevels = Array.from(Array(MAX_TH_LEVEL)).map((_, idx) => (MAX_TH_LEVEL - idx));
        return {
            valueInputOption: "USER_ENTERED",
            data: [
                {
                    range: `${SUMMARY_SHEET_NAME}!A1:${column}${leaguegroup.clans.length + 1}`,
                    values: [
                        [
                        ...
                        ...leaguegroup.clans.map((clan: any) => {
                            return [
                                ...
                                ...townhallLevels.map((level) => {
                                    const len = clan.members.filter((member: any) => member.townHallLevel == level).length
    ...
    public static initializeClanSheetRequestBody(leaguegroup: any): sheets_v4.Schema$BatchUpdateValuesRequest {

        const opponentsCount = leaguegroup.clans.length - 1;
        const rounds = Array.from(Array(opponentsCount)).map((_, idx) => `R${idx + 1}`);
        const columnStart = String.fromCharCode('A'.charCodeAt(0) + CLAN_SHEET_HEADER.length)
        const columnEnd = String.fromCharCode(columnStart.charCodeAt(0) + opponentsCount - 1);
        return {
            valueInputOption: "USER_ENTERED",
            data: [
                ...leaguegroup.clans.map((clan: any) => {
